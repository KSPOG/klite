...
public class ShortestPathRoutePlannerTest
{
	@Test
	public void producesOrderedAdjacentShortestRoute()
	{
		ShortestPathRoutePlanner planner = new ShortestPathRoutePlanner(
			new TestMap(0, 0, 12, 12));

		PathSearchResult result = planner.find(point(1, 1), point(6, 4));
		...
		assertAdjacent(result.getPath());
	}

	@Test
	public void routesAroundBlockedTilesWithoutLeavingRouteGaps()
	{
		TestMap map = new TestMap(0, 0, 12, 12);
		map.block(3, 1);
		map.block(3, 2);
		map.block(3, 3);
		ShortestPathRoutePlanner planner = new ShortestPathRoutePlanner(map);

		PathSearchResult result = planner.find(point(1, 2), point(6, 2));
	...
	@Test
	public void reachesWalkableTileInsideArrivalRadius()
	{
		TestMap map = new TestMap(0, 0, 12, 12);
		WorldPoint destination = point(8, 8);
		map.block(destination.getX(), destination.getY());
		ShortestPathRoutePlanner planner = new ShortestPathRoutePlanner(map);

		PathSearchResult result = planner.find(point(1, 1), destination, 2);

		assertTrue(result.isReached());
		WorldPoint reached = result.getPath().get(result.getPath().size() - 1);
		assertFalse(map.isBlocked(reached));
		assertTrue(reached.distanceTo(destination) <= 2);
		assertAdjacent(result.getPath());
	}

	private static void assertAdjacent(List<WorldPoint> route)
	{
		for (int index = 1; index < route.size(); index++)
		{
			assertEquals(1, route.get(index - 1).distanceTo(route.get(index)));
		}
	}
